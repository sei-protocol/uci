...
**seidroid feature** — its base prompts, its driver, and/or the composite action it
...
| [`xreview/`](xreview/) | On-demand, sandbox-backed deep review. Drives the `sei-droid` agent inside a managed omnigent Kubernetes sandbox with a real `git`/`gh` toolchain, so it can build, test, and inspect the tree before returning one structured verdict. A Python session driver plus a composite action. Ships dry-run by default. | `seidroid xreview` PR comment |
